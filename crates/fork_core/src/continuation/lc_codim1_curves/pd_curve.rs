...
impl<'a> PDCurveProblem<'a> {
    ...
    pub fn debug_initial_residual(
        ...
        let colloc_norm: f64 = residual
            .rows(0, colloc_end)
            .iter()
            .map(|x| x * x)
            .sum::<f64>()
            .sqrt();
        let cont_norm: f64 = residual
            .rows(cont_start, self.ntst * self.dim)
            .iter()
            .map(|x| x * x)
            .sum::<f64>()
            .sqrt();
        let phase_val = if phase_row < n_eqs {
            residual[phase_row]
        } else {
            0.0
        };
        ...
        logs.push(format!(
            "  Collocation:     {:.6e} (rows 0..{})",
            colloc_norm, colloc_end
        ));
        logs.push(format!(
            "  Continuity:      {:.6e} (rows {}..{})",
            cont_norm, cont_start, cont_end
        ));
        logs.push(format!(
            "  Phase condition: {:.6e} (row {})",
            phase_val, phase_row
        ));
    ...
    fn build_periodic_jac(&mut self, aug: &DVector<f64>) -> Result<DMatrix<f64>> {
        ...
        for i in 0..=self.ntst {
            ...
            for d in 0..self.dim {
                jac[(phase_row, mesh_col + d)] =
                    self.upoldp[upoldp_start + d] * h / (self.ntst + 1) as f64;
            }
        }
...
mod tests {
    ...
    fn get_p2_returns_error_on_bounds_mismatch() {
        ...
        let problem = PDCurveProblem::new(&mut system, lc_state, 1.0, 0, 1, 0.0, 0.0, 1, 1)
