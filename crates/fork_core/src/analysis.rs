...
/// Streaming Welch-style scalar power spectrum accumulator.
///
/// The accumulator is intentionally source-agnostic: callers can push chunks
/// read from disk, samples generated by a flow/map trajectory, or samples
/// derived from any future data source that yields a scalar observable.
...
impl PowerSpectrumAccumulator {
    pub fn new(sample_interval: f64, window_size: usize) -> Result<Self> {
        ...
        if !window_size.is_power_of_two() {
            bail!("window_size must be a power of two.");
        }
        ...
        Ok(Self {
            ...
            accumulated_power: vec![0.0; window_size / 2 + 1],
            fft,
            sample_count: 0,
            segment_count: 0,
        })
    ...
    pub fn push_samples(&mut self, samples: &[f64]) -> Result<()> {
        for &sample in samples {
            if !sample.is_finite() {
                bail!("Power spectrum samples must be finite.");
            }
            self.buffer.push(sample);
            self.sample_count += 1;
            if self.buffer.len() == self.window_size {
                self.accumulate_current_window();
                self.buffer.clear();
            }
        }
        Ok(())
    ...
    pub fn finish(mut self) -> PowerSpectrumResult {
        if self.segment_count == 0 && self.buffer.len() >= 2 {
            self.accumulate_partial_window();
        }

        let bin_count = self.accumulated_power.len();
        let frequency_step = 1.0 / (self.sample_interval * self.window_size as f64);
        let frequencies = (0..bin_count)
            .map(|index| index as f64 * frequency_step)
            .collect::<Vec<_>>();
        let scale = if self.segment_count > 0 {
    ...
    fn accumulate_partial_window(&mut self) {
        let mut padded = vec![0.0; self.window_size];
        let len = self.buffer.len();
        padded[..len].copy_from_slice(&self.buffer);
        self.accumulate_window(&padded);
    }

    fn accumulate_current_window(&mut self) {
        let window = self.buffer.clone();
        self.accumulate_window(&window);
    }

    fn accumulate_window(&mut self, samples: &[f64]) {
        ...
        let mut input = samples
            ...
            .map(|(index, value)| {
                let phase =
                    (2.0 * std::f64::consts::PI * index as f64) / (self.window_size - 1) as f64;
        ...
        let window_energy = hann_window_energy(self.window_size);
        let normalization = if window_energy > 0.0 {
            1.0 / (window_energy * self.window_size as f64)
        } else {
            0.0
        };

        for (index, bin) in input.iter().take(self.window_size / 2 + 1).enumerate() {
            self.accumulated_power[index] += bin.norm_sqr() * normalization;
        }
...
pub fn power_spectrum(
    ...
    let mut accumulator = PowerSpectrumAccumulator::new(sample_interval, window_size)?;
    accumulator.push_samples(samples)?;
    Ok(accumulator.finish())
}

fn hann_window_energy(window_size: usize) -> f64 {
    if window_size <= 1 {
        return 0.0;
    }
    (0..window_size)
        .map(|index| {
            let phase = (2.0 * std::f64::consts::PI * index as f64) / (window_size - 1) as f64;
            let hann = 0.5 * (1.0 - phase.cos());
            hann * hann
        })
        .sum()
...
mod tests {
    use super::{
        ...
        power_spectrum, solve_upper, thin_qr_positive, LyapunovStepper, PowerSpectrumAccumulator,
    };
    ...
    #[test]
    fn power_spectrum_detects_dominant_sine_frequency() {
        let sample_interval = 0.01;
        let frequency = 12.5;
        let samples = (0..256)
            .map(|index| {
                let t = index as f64 * sample_interval;
                (2.0 * std::f64::consts::PI * frequency * t).sin()
            })
            .collect::<Vec<_>>();

        let result =
            power_spectrum(&samples, sample_interval, 256).expect("power spectrum should compute");

        let dominant_bin = result
            .power
            .iter()
            .enumerate()
            .max_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap())
            .map(|(index, _)| index)
            .expect("spectrum should have bins");
        assert!((result.frequencies[dominant_bin] - frequency).abs() < 0.5);
        assert_eq!(result.sample_count, samples.len());
        assert_eq!(result.segment_count, 1);
    }

    #[test]
    fn power_spectrum_accumulator_accepts_streamed_chunks() {
        let samples = (0..64).map(|index| index as f64).collect::<Vec<_>>();
        let mut accumulator =
            PowerSpectrumAccumulator::new(1.0, 32).expect("accumulator should initialize");
        accumulator
            .push_samples(&samples[..17])
            .expect("first chunk should push");
        accumulator
            .push_samples(&samples[17..])
            .expect("second chunk should push");
        let result = accumulator.finish();
        assert_eq!(result.sample_count, 64);
        assert_eq!(result.segment_count, 2);
        assert_eq!(result.power.len(), 17);
    }

    #[test]
    fn power_spectrum_rejects_invalid_stream_settings() {
        let message = match PowerSpectrumAccumulator::new(0.0, 32) {
            Ok(_) => panic!("expected error"),
            Err(err) => format!("{err}"),
        };
        assert!(message.contains("sample_interval must be positive"));
        let message = match PowerSpectrumAccumulator::new(1.0, 30) {
            Ok(_) => panic!("expected error"),
            Err(err) => format!("{err}"),
        };
        assert!(message.contains("window_size must be a power of two"));
        let mut accumulator =
            PowerSpectrumAccumulator::new(1.0, 32).expect("accumulator should initialize");
        assert_err_contains(
            accumulator.push_samples(&[1.0, f64::NAN]),
