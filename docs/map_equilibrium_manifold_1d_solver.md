...
2. Build initial fundamental domain:
   - unstable: from `x_seed` to `F^n(x_seed)`
   - stable: from `x_seed` to a Newton preimage `y` solving `F^n(y) = x_seed`
3. Grow domain iteratively:
   - positive multiplier: advance with `F^n`
   - negative multiplier: advance a directed half-branch with `F^(2n)` so `Plus` and `Minus` do not alternate and overlap
   - stable: use the corresponding Newton preimage solves
...
- For stable runs, preimage solves use the Jacobian of `F^n` and damped Newton line search.
