export type HomoclinicExtraFlags = {
  freeTime: boolean
  freeEps0: boolean
  freeEps1: boolean
}

export type HomoclinicExtraKey = keyof HomoclinicExtraFlags
...
function selectedExtraCount(flags: HomoclinicExtraFlags): number {
  return Number(flags.freeTime) + Number(flags.freeEps0) + Number(flags.freeEps1)
}
...
export function homoclinicExtraSelectionError(
  ...
  const selected = selectedExtraCount({ freeTime, freeEps0, freeEps1 })
  if (selected === 0) {
...
export function isHomoclinicExtraSelectionDisabled(
  flags: HomoclinicExtraFlags,
  key: HomoclinicExtraKey
): boolean {
  return !flags[key] && selectedExtraCount(flags) >= 2
}
