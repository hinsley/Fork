...
export type TransferOperatorRequest = { system: SystemConfig; axes: StateGridAxisRequest[]; startingPoint: number[]; samplesPerCell: number; iterations: number; timeStep: number; maxStationaryIterations: number; tolerance: number }
...
export type EquilibriumManifold1DGroupExtensionRequest = {
  system: SystemConfig
  branchDataList: ContinuationBranchDataWire[]
  mapIterations: number
  settings: EquilibriumManifold1DSettingsRequest
}

export type EquilibriumManifold1DGroupExtensionResult = ContinuationBranchData[]
