...
import { clickInspectorAction, createHarness, revealInspectorAction } from './harness'
...
test('continues, reloads, and extends a real two-equilibrium heteroclinic curve', async ({
  ...
  await revealInspectorAction(page, 'action-heteroclinic-from-orbit-toggle')
  ...
  await expect(page.getByText(/heteroclinic curve · 3 points/i)).toBeVisible()
  await harness.openDisclosure('branch-summary-toggle')
  const solverDetails = page.getByTestId('branch-summary-toggle').locator('..')
  await expect(solverDetails.getByText('Multiple shooting', { exact: true })).toBeVisible()
  await expect(solverDetails.getByText('6', { exact: true })).toBeVisible()
  await harness.openDisclosure('branch-points-toggle')
  await page.getByTestId('branch-point-input').fill('1')
  await page.getByTestId('branch-point-jump').click()
  await page.getByTestId('branch-point-details-toggle').click()
